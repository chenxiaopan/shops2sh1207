...
<script type="text/javascript">
    ...
    function emptyForder(){
    document.location.href="${shop}/forder_emptyForder.action";
    }

	function remove(tt) {
		//通过获得<tr>标签里的lang属性值获取要删除的购物项的商品id，因为此时订单还没结算，所以购物项没有id
		var id = $(tt).parent().parent().attr("lang");
		document.location.href="${shop}/sorder_deleteSorder.action?product.id="+id;
	};
			...
			<div class="header_container">
				...
				<div class="top_bar clear">
					...
					<ul class="top_links fr">
						...
						<c:choose>
							<c:when test="${session.user==null}">
								<li><a href="#">注册</a></li>
								<li><a href="${shop}/loginForm">登录</a></li>
							</c:when>
							<c:when test="${session.user!=null}">
								<li><a href="#">欢迎您~ ${session.user.uname}</a></li>
								<li><a href="${shop }/user_logout">退出登录</a></li>
							</c:when>
						</c:choose>
					</ul>
					<!--头部小导航结束-->
					...
					<h1 class="logo clear fl">
						<a href="${shop }/index.jsp"> <img
							src="${shop}/images/logo.png" /> </a>
					</h1>
					<!-- 小购物车 -->
					<div class="minicart">
						<a class="minicart_link" href="#"> <span class="item">
								<b>${fn:length(sessionScope.forder.sorders)}</b> 件/ </span> <span
							class="price"> <b>￥${sessionScope.forder.total}</b> </span> </a>
					</div>
					<!-- 小购物车结束 -->
					<!-- 搜索框 -->
					<div class="header_search">
						<div class="form-search ">
							<input value="请输入商品名称" class="input-text" type="text" id="search" />
							<button type="button" title="Search" onclick="javascript:searchShowCart()"></button>
						</div>
					</div>
				</div>
			</div>
				...
				<table class="data-table cart-table" cellpadding="0" cellspacing="0">
					...
					<c:forEach items="${sessionScope.forder.sorders }" var="sorder"
						...
						<tr lang="${sorder.product.id}">
							<td class="align_center"><a href="#" class="edit">${num.count
									}</a>
							</td>
							<td width="80px"><img
								src="${shop}/upload/${sorder.product.pic}" width="80" height="80" />
							</td>
							<td class="align_left"><a class="pr_name" href="#">${sorder.name
									}</a>
							</td>
							<td class="align_center vline">${sorder.price }</td>
							<td class="align_center vline">
								<!-- 文本框 --> <input class="text" style="height: 20px;"
								value="${sorder.numbers }" lang="${sorder.numbers }">
							</td>
							<td class="align_center vline">
								${sorder.price*sorder.numbers }</td>
							<td class="align_center vline"><a href="javascript:void(0);"
								onclick="javascript:remove(this)" class="remove"></a>
							</td>
						</tr>
					</c:forEach>
						...
						<tbody>
							<tr>
								...
								<td class="align_right" style=""><strong>￥<span
										class="price" id="total">${sessionScope.forder.total}</span> </strong>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td class="align_right" style="">￥<span class="total"
									id="totalAll"><strong>${sessionScope.forder.total}</strong>
								</span>
								</td>
							</tr>
						</tbody>
					...
					<div class="action_buttonbar">
						<font><a href="${shop}/user/confirm.jsp">
								<button type="button" title="" class="checkout fr"
									style="background-color: #f38256;">订单确认</button> </a> </font>
						<button type="button" title="" class=" fr" onclick="javascript:emptyForder()">
