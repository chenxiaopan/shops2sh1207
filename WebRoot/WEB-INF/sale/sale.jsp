...
<script type="text/javascript">
	$(function() {
		$("#submit").click(
				function() {
					var colorArr = [ '#44A622', '#A7B629', '#CAD857',
							'#E4DB7B', '#ECDE49', '#ECC049', '#EC9649',
							'#D97431', '#D95531', '#E4DB7B' ];
					//发送Ajax请求
					$.post($("#sale").val(), {
						numbers : $("#numbers").val()
					}, function(data) {
						...
						//将json对象数组转为二维数组，否则会报数据格式错误
						for (i = 0; i < data.length; i++) {
							myData[i] = [ data[i].name, data[i].numbers ];
						}
						var myChart = new JSChart('chart_container', $("#type")
								.val(), '');
						myChart.setDataArray(myData);
						myChart.colorize(colorArr.slice(0, data.length));
						myChart.setSize(100 * $("#numbers").val(), 400);
