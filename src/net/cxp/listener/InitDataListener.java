...
public class InitDataListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent event) {
		...
		// 把servletContext内置对象交给productTimerTask
		productTimerTask.setApplication(event.getServletContext());

		// @param isDaemon true if the associated thread should run as a daemon.
		// 通过设置定时器，让首页的数据每隔一小时同步一次（配置为守护线程）
		new Timer(true).schedule(productTimerTask, 0, 1000 * 60 * 60 * 6);// 每个6个一小时执行一次productTimerTask

		
		
		/*
		 * //将存储银行图片的数组放到application中，项目启动的时候加载 fileUpload = (FileUpload)
		 * context.getBean("fileUpload");
